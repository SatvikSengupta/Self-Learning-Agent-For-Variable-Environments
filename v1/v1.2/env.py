...
class VisionArmEnv(gym.Env):
    ...
    def step(self, action):
        ...
        # Apply the brain's decision to the motors
        p.setJointMotorControlArray(self.armId, jointIndices=joint_indices, 
                                    controlMode=p.POSITION_CONTROL, targetPositions=action)
        
        # --- THE FIX: Frame Skipping ---
        # Step the physics engine 10 times holding the SAME action 
        # so the virtual servos actually have time to move.
        for _ in range(10):
            p.stepSimulation()
            time.sleep(1./2400.) # Extremely fast sleep just to keep the UI from freezing
